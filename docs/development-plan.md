...
P2
已完成 ✅
已接入 scheduler.ts 每小时第 5 分钟触发 handleMetricsCollection（见规范 5.3 P2 已剔除历史完成项）
迁移文件时间戳规范化
P2
已完成 ✅
012 重命名为 030，018 → 031/032 等已落地，无时间戳冲突（见规范 5.3 P2 已剔除历史完成项）
全页面样式精修、移动端适配统一
P3
持续滚动
...
- 前端 P2 setState 泄漏低概率场景（Auth 4 件套 / Create 表单页 navigate 跳转后 setState / Profile / Admin/SystemConfig 异步提交路径均已加 mountedRef 守卫，累计覆盖 9 个页面，setState 卸载泄漏全量清零）

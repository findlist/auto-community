 ...
 * 3. 仅 ApiError 与字符串返回其内容；原生 Error（网络异常/超时等）的 message
 *    多为技术性信息（如 "Network Error"），不适合直接展示给用户，统一走 fallback
 *
...
export function getErrorMessage(err: unknown, fallback = "操作失败，请稍后重试"): string {
  // 业务错误：ApiError 携带后端返回的可读 message
  ...
  // 原生 Error 与未知类型统一走 fallback，避免技术性错误信息泄露给用户
  return fallback;
}

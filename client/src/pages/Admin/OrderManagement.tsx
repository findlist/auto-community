...
export default function OrderManagement() {
  ...
  // 竞态守卫：跟踪当前活跃的请求标识（type+status+page），快速切换时旧请求返回不再覆盖新数据
  // 设计原因：loadOrders 依赖 type/status/page，切换时旧请求的 await 完成后会 setList 旧列表覆盖新列表
  ...
  const loadOrders = useCallback(async (t: string, s: string, p: number) => {
    ...
    try {
      ...
      // 竞态守卫：await 期间若 type/status/page 已变化，跳过 setState 避免旧数据覆盖新数据
    ...
    <div>
      ...
      {loading ? (
        ...
        <Empty
          ...
          action={
            <button
              onClick={() => loadOrders(type, status, page)}
